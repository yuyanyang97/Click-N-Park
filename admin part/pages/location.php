                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                <?php $location_result = mysqli_query($conn,"SELECT * from location");
                                $i=1;
                                while($location_rows = mysqli_fetch_assoc($location_result))
                                    { ?>
                                    <tr class="odd gradeX">
                                        ...
                                        <td><?php echo $i++ ?></td>
                                        <td><?php echo $location_rows["Location_Name"] ?></td>
                                        <td><?php echo $location_rows["Area_ID"] ?></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
							<br />
							<button type="button" id="showlocation" class="btn btn-default">Add Location</button>
							<div id="showNewLocation" style="width:250px;display:none;margin-top:5px;">
                            <form id="addnewlocation">
								<p>Enter a new location: </p>
								<input type="text" class="form-control" id="newlocation" name="newlocation" width="200px" />
								<br />
								<p>Which area : 
								<select id="newarea" name="newarea">
                                <option value=""> </option>
                                    <?php $area_result = mysqli_query($conn,"SELECT * from area");
                                    while($area_rows = mysqli_fetch_assoc($area_result))
                                    { ?>
    								<option value="<?php echo $area_rows["Area_ID"]; ?>"><?php echo $area_rows["Area_Name"]; ?></option>
                                    <?php } ?>
								</select>
								</p>
								<br />
    ...
    $(document).ready(function(){
        ...
        rules:{
            ...
            messages:{
              ...
              newarea:{
                required: "Please select the area of location."
              }
            },
        submitHandler: function(form) {
        $.ajax({
            type:"POST",
            data: {location: $('#newlocation').val(), area: $('#newarea').val()},
            ...
            success: function(respone){
                ...
                else
                {
                    $(document).ready(function(){
                    bootbox.alert({
                        message: "The location is existed",
                        size: 'small'
                    });
                });
                }
