        ...
        <input type="button" onclick="window.location = 'summon_history.php';" class="btn btn-primary" id="summon" value="Summon History"/>
            ...
            <form name="parkhis" method="get" action="print_parkhis.php">
                    ...
                    <tr>
                        ...
                        <th class="col-md-2 col-xs-5">Start Parking</th>
                        <th class="col-md-2 col-xs-5">Stop Parking</th>
                        <th class="col-md-5 col-xs-5">Duration(min)</th>
                        <th class="col-md-5 col-xs-5">Rate(RM)</th>
                        <th class="col-md-5 col-xs-5">Amount(RM)</th>
                    </tr>
                    </thead>
                    <tbody>
                        <?php 
                        $result_park =mysqli_query($conn, "SELECT * FROM (((park
                        INNER JOIN location ON park.Location_ID = location.Location_ID)
                        INNER JOIN car ON park.Car_ID = car.Car_ID) INNER JOIN rate ON park.Rate_ID=rate.Rate_ID)
                        where park.Driver_ID=$sess_memid");
                        while($show_park = mysqli_fetch_assoc($result_park))
                        { 
                        ?>
                        <tr>
                            <td><input type="checkbox" name="print[]" id="print" value="<?php echo $show_park["Park_ID"] ?>"/></td>
                            <td><?php $date = new DateTime($show_park["Park_Date"]);
                            echo $date->format('d-m-Y'); ?></td>
                            <td><?php echo $show_park["Car_plat_No"] ?></td>
                            <td><?php echo $show_park["Location_Name"] ?></td>
                            <td><?php echo $show_park["Park_TimeIN"] ?></td>
                            <td><?php echo $show_park["Park_TimeOut"] ?></td>
                            <td><?php echo $show_park["Park_Duration"] ?></td>
                            <td><?php echo $show_park["rate_Cost"] ?></td>
                            <td><?php echo $show_park["Park_Amount"] ?></td>
                        </tr>
                        <?php } ?>
                    </tbody>
            </table>
            <input type="submit" name="printbtn" class="btn btn-primary" value="Print" id="printbtn"/>
            </form>
            </div>
